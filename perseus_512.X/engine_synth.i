...
# 41 "../src/audio/engine_synth.c" 2
# 59 "../src/audio/engine_synth.c"
static enginesynth_wrap_t s_eng;
static engine_blip_state_t s_blip;
       
# 61 "../src/audio/engine_synth.c" 3 4
      _Bool 
# 61 "../src/audio/engine_synth.c"
                           Start_Blip = 
# 61 "../src/audio/engine_synth.c" 3 4
                                        0
# 61 "../src/audio/engine_synth.c"
...
void app_engine_synth_init(void)
{
    ...
    EngineSynthWrap_init(&s_eng, (48000), ENG_V8, 0 , 1 , 0.09f , (2));

    EngineSynth_setRPM(&s_eng.core, 0.0f);
...
void app_engine_synth_process(float* p_in, float* p_out)
{
    ...
    uint16_t pot = POT_Read();
    float pot_rpm = (float)(pot * 2);


    float dt = (float)(32) / (float)(48000);
    ...
    if ( Start_Blip )
    {
        ...
        Start_Blip = 
# 99 "../src/audio/engine_synth.c" 3 4
                    0
# 99 "../src/audio/engine_synth.c"
                         ;
    }
    ...
    engine_process(&s_eng, p_in, p_out, (32));
}
# 128 "../src/audio/engine_synth.c"
...
void EngineBlip_init(engine_blip_state_t* b)
{
    ...
    b->p.peak_rpm_default = 5000.0f;

    b->p.attack_ms = 500.0f;

    b->p.hold_ms = 400.0f;

    b->p.decay_ms = 1300.0f;

    b->p.cooldown_ms = 0.0f;
    b->p.attack_curve = 1.4f;

    b->p.decay_curve = 0.1f;
...
float EngineBlip_update(engine_blip_state_t* b, float pot_rpm, float dt_sec)
{
    ...
    return clampf(rpm_cmd, (200.0f), 8000.0f);
}
# 298 "../src/audio/engine_synth.c"
...
# 376 "../src/audio/engine_synth.c"
    ...
    case ENG_V8: w->params.eo = 2.0f; s->numHarm = 8; break;
...
void engine_process(enginesynth_wrap_t* w, float* p_in, float* p_out, int samples)
{
    ...
    const float RPM_TILT_AM_TH = 4000.0f;

    const float TILT_DELTA_DB_OCT = -2.0f;
    const int HIHARM_START_K = 6;
    const float HIHARM_SLOPE_DB = -1.0f;

    const float AM_RUMBLE_SCALE_HI = 0.7f;


    const float RPM_NOISE_START = 4600.0f;
    const float RPM_NOISE_END = 6400.0f;
    const float NOISE_BOOST_DB = +8.0f;
    const float NOISE_FC_MULT_MAX = 1.25f;
    const float NOISE_PULSE_MIX = 0.7f;
    ...
    struct { int k_max; float gain_k[16]; } pc;

    {
        ...
        for (int k = 1; k <= kmax; k++) {
            ...
            {
                add_dB += TILT_DELTA_DB_OCT * oct;
                if (k > HIHARM_START_K)
                {
                    add_dB += HIHARM_SLOPE_DB * (float)(k - HIHARM_START_K);
                }
            }
            else
            {
    ...
    {
        float A = powf(10.0f, EQ_GAIN_DB * 0.05f);
        float w0 = 2.0f * (float)
# 661 "../src/audio/engine_synth.c" 3 4
                                 3.14159265358979323846 
# 661 "../src/audio/engine_synth.c"
                                      * EQ_FC_HZ / fs;
        float c = 
# 662 "../src/audio/engine_synth.c" 3 4
                   __builtin_cosf
# 662 "../src/audio/engine_synth.c"
                       (w0);
        float s0 = 
# 663 "../src/audio/engine_synth.c" 3 4
                   __builtin_sinf
# 663 "../src/audio/engine_synth.c"
                       (w0);
...

    {
        ...
        float w0 = 2.0f * (float)
# 697 "../src/audio/engine_synth.c" 3 4
                                 3.14159265358979323846 
# 697 "../src/audio/engine_synth.c"
                                      * w->lp_fc_smooth / fs;
        float c = 
# 698 "../src/audio/engine_synth.c" 3 4
                   __builtin_cosf
# 698 "../src/audio/engine_synth.c"
                       (w0);
        float s0 = 
# 699 "../src/audio/engine_synth.c" 3 4
                   __builtin_sinf
# 699 "../src/audio/engine_synth.c"
                       (w0);
...
# 728 "../src/audio/engine_synth.c"
    const float detune_mix_gain = powf(10.0f, (-6.0f) / 20.0f);



    static float detune_jitter_c = 0.0f;
    ...
    for (int smp = 0; smp < samples; smp++)
    {
        ...
        if (rpm <= 2000.0f) detune_c = 3.0f;
            ...
            float uu = (rpm - 2000.0f)/3000.0f;
            detune_c = 3.0f + (5.0f - 3.0f)*uu;
        } else {
            float uu = (rpm - 5000.0f)/2000.0f;
            detune_c = 5.0f + (6.5f - 5.0f)*uu;
        }

        jitter_phase += 2.0f*(float)
# 756 "../src/audio/engine_synth.c" 3 4
                                   3.14159265358979323846 
# 756 "../src/audio/engine_synth.c"
                                        * 4.0f / fs;
        if (jitter_phase >= 2.0f*(float)
# 757 "../src/audio/engine_synth.c" 3 4
                                       3.14159265358979323846
# 757 "../src/audio/engine_synth.c"
                                           ) {
            jitter_phase -= 2.0f*(float)
# 758 "../src/audio/engine_synth.c" 3 4
                                       3.14159265358979323846
# 758 "../src/audio/engine_synth.c"
                                           ;
            float r = (( w->rng ^= (w->rng<<13), w->rng ^= (w->rng>>17), w->rng ^= (w->rng<<5), ((w->rng & 0xFFFFFFFFu) * 2.3283064365386963e-10f) )*2.0f - 1.0f);



            detune_jitter_c = r * 1.5f;

        }
        detune_c += detune_jitter_c;

        const float detune_ratio = powf(2.0f, detune_c / 1200.0f);
        ...
        w->lfo1_phase += 2.0f*(float)
# 775 "../src/audio/engine_synth.c" 3 4
                                    3.14159265358979323846 
# 775 "../src/audio/engine_synth.c"
                                         * lfo1_f / fs;
        w->lfo2_phase += 2.0f*(float)
# 776 "../src/audio/engine_synth.c" 3 4
                                    3.14159265358979323846 
# 776 "../src/audio/engine_synth.c"
                                         * lfo2_f / fs;
        w->rum_phase += 2.0f*(float)
# 777 "../src/audio/engine_synth.c" 3 4
                                    3.14159265358979323846 
# 777 "../src/audio/engine_synth.c"
                                         * rum_f / fs;
        w->sub_phase += 2.0f*(float)
# 778 "../src/audio/engine_synth.c" 3 4
                                    3.14159265358979323846 
# 778 "../src/audio/engine_synth.c"
                                         * (0.25f * f0) / fs;

        if (w->lfo1_phase >= 2.0f*(float)
# 780 "../src/audio/engine_synth.c" 3 4
                                        3.14159265358979323846
# 780 "../src/audio/engine_synth.c"
                                            ) w->lfo1_phase -= 2.0f*(float)
# 780 "../src/audio/engine_synth.c" 3 4
                                                                           3.14159265358979323846
# 780 "../src/audio/engine_synth.c"
                                                                               ;
        if (w->lfo2_phase >= 2.0f*(float)
# 781 "../src/audio/engine_synth.c" 3 4
                                        3.14159265358979323846
# 781 "../src/audio/engine_synth.c"
                                            ) w->lfo2_phase -= 2.0f*(float)
# 781 "../src/audio/engine_synth.c" 3 4
                                                                           3.14159265358979323846
# 781 "../src/audio/engine_synth.c"
                                                                               ;
        if (w->rum_phase >= 2.0f*(float)
# 782 "../src/audio/engine_synth.c" 3 4
                                        3.14159265358979323846
# 782 "../src/audio/engine_synth.c"
                                            ) w->rum_phase -= 2.0f*(float)
# 782 "../src/audio/engine_synth.c" 3 4
                                                                           3.14159265358979323846
# 782 "../src/audio/engine_synth.c"
                                                                               ;
        if (w->sub_phase >= 2.0f*(float)
# 783 "../src/audio/engine_synth.c" 3 4
                                        3.14159265358979323846
# 783 "../src/audio/engine_synth.c"
                                            ) w->sub_phase -= 2.0f*(float)
# 783 "../src/audio/engine_synth.c" 3 4
                                                                           3.14159265358979323846
# 783 "../src/audio/engine_synth.c"
        ...
        float wobble_raw = lfo1_d * 
# 785 "../src/audio/engine_synth.c" 3 4
                                   __builtin_sinf
# 785 "../src/audio/engine_synth.c"
                                       (w->lfo1_phase)
                         + lfo2_d * 
# 786 "../src/audio/engine_synth.c" 3 4
                                   __builtin_sinf
# 786 "../src/audio/engine_synth.c"
                                       (w->lfo2_phase)
                         + 0.016f * 
# 787 "../src/audio/engine_synth.c" 3 4
                                   __builtin_sinf
# 787 "../src/audio/engine_synth.c"
                                       (w->rum_phase);

        const float W_MAX = 0.05f;
        float wobble = 1.0f + W_MAX * tanhf(wobble_raw / W_MAX);
        f0 *= wobble;


        float dphi0 = 2.0f * (float)
# 794 "../src/audio/engine_synth.c" 3 4
                                   3.14159265358979323846 
# 794 "../src/audio/engine_synth.c"
                                        * f0 / fs;
        s->phase0 += dphi0;
        if (s->phase0 >= 2.0f*(float)
# 796 "../src/audio/engine_synth.c" 3 4
                                    3.14159265358979323846
# 796 "../src/audio/engine_synth.c"
                                        ) s->phase0 -= 2.0f*(float)
# 796 "../src/audio/engine_synth.c" 3 4
                                                                   3.14159265358979323846
# 796 "../src/audio/engine_synth.c"
        ...
        for (int k = 1; k <= pc.k_max; k++) {
            yA += pc.gain_k[k - 1] * 
# 801 "../src/audio/engine_synth.c" 3 4
                                    __builtin_sinf
# 801 "../src/audio/engine_synth.c"
                                        (s->phase0 * (float)k);
        }
        ...
        if (s->type == ENG_V8) {
            ...
            float dphiB = 2.0f * (float)
# 808 "../src/audio/engine_synth.c" 3 4
                                       3.14159265358979323846 
# 808 "../src/audio/engine_synth.c"
                                            * fB / fs;
            w->phase_bankB += dphiB;
            if (w->phase_bankB >= 2.0f*(float)
# 810 "../src/audio/engine_synth.c" 3 4
                                             3.14159265358979323846
# 810 "../src/audio/engine_synth.c"
                                                 ) w->phase_bankB -= 2.0f*(float)
# 810 "../src/audio/engine_synth.c" 3 4
                                                                                 3.14159265358979323846
# 810 "../src/audio/engine_synth.c"
                                                                                     ;
            ...
            for (int k = 1; k <= pc.k_max; k++) {
                yB += pc.gain_k[k - 1] * 
# 814 "../src/audio/engine_synth.c" 3 4
                                        __builtin_sinf
# 814 "../src/audio/engine_synth.c"
                                            (w->phase_bankB * (float)k);
            }
...

        {
            ...
            if (rpm >= RPM_TILT_AM_TH) {
                AM_RUMBLE_DEPTH_eff *= AM_RUMBLE_SCALE_HI;
            }
            float am_env = 1.0f
                         + AM_RUMBLE_DEPTH_eff * 
# 827 "../src/audio/engine_synth.c" 3 4
                                                __builtin_sinf
# 827 "../src/audio/engine_synth.c"
                                                    (w->rum_phase)
                         + AM_SUB_DEPTH * 
# 828 "../src/audio/engine_synth.c" 3 4
                                                __builtin_sinf
# 828 "../src/audio/engine_synth.c"
                                                    (w->sub_phase);
...

        {
            ...
            float gate_sine = 0.5f + 0.5f * 
# 842 "../src/audio/engine_synth.c" 3 4
                                           __builtin_sinf
# 842 "../src/audio/engine_synth.c"
                                               (w->rum_phase);
            float gate_half = fmaxf(0.0f, 
# 843 "../src/audio/engine_synth.c" 3 4
                                         __builtin_sinf
# 843 "../src/audio/engine_synth.c"
                                             (w->rum_phase));
            float gate = (1.0f - NOISE_PULSE_MIX) * gate_sine + NOISE_PULSE_MIX * gate_half;


            float uwhite = (( w->rng ^= (w->rng<<13), w->rng ^= (w->rng>>17), w->rng ^= (w->rng<<5), ((w->rng & 0xFFFFFFFFu) * 2.3283064365386963e-10f) ) * 2.0f - 1.0f) * gate;


            float fc_eff_mult = 1.0f + (NOISE_FC_MULT_MAX - 1.0f) * u;
            float noise_alpha_eff = expf(-2.0f * (float)
# 851 "../src/audio/engine_synth.c" 3 4
                                                       3.14159265358979323846
                                         
# 852 "../src/audio/engine_synth.c"
