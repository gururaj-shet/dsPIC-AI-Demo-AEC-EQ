...
extern void tone_process( tone_t* ptone,
                           const float* in,
                                 float* out,
                                 int frameSize );
# 18 "../src/audio/tone_ctrl.c" 2
# 85 "../src/audio/tone_ctrl.c"
           tone_t My_ToneTre;
           tone_t My_ToneMid;
           tone_t My_ToneBas;
...
{
    tone_init(&My_ToneTre, 100, 0.0f, (1.2f), 1, ((2)));
    tone_init(&My_ToneMid, 100, 0.0f, (1.2f), 0, ((2)));
    tone_init(&My_ToneBas, 100, 0.0f, (1.2f), 0, ((2)));




    tone_set_coeffs(&My_ToneTre, 0.0f, (8000.0f));


    tone_set_coeffs(&My_ToneMid, 0.0f, (1000.0f));


    tone_set_coeffs(&My_ToneBas, 0.0f, (125.0f));
}


void app_tone_set_coeffs_tre( float gain_dB )
{
    tone_set_coeffs(&My_ToneTre, gain_dB, (8000.0f));
}
void app_tone_set_coeffs_mid( float gain_dB )
{
    tone_set_coeffs(&My_ToneMid, gain_dB, (1000.0f));
}
void app_tone_set_coeffs_bas( float gain_dB )
{
    tone_set_coeffs(&My_ToneBas, gain_dB, (125.0f));
}


void app_tone_process_tre( const float* in, float* out )
{
    tone_process(&My_ToneTre, in, out, (32));
}
void app_tone_process_mid( const float* in, float* out )
{
    tone_process(&My_ToneMid, in, out, (32));
}
void app_tone_process_bas( const float* in, float* out )
{
    tone_process(&My_ToneBas, in, out, (32));
}
# 158 "../src/audio/tone_ctrl.c"
...
void tone_set_coeffs(tone_t* ptone, float gain_dB, float center_Hz)
{
    ...
    w0 = 2.0f * 
# 204 "../src/audio/tone_ctrl.c" 3 4
                   3.14159265358979323846 
# 204 "../src/audio/tone_ctrl.c"
                        * center_Hz / sample_rate;
    cos_w0 = 
# 205 "../src/audio/tone_ctrl.c" 3 4
            __builtin_cosf
# 205 "../src/audio/tone_ctrl.c"
                (w0);
    sin_w0 = 
# 206 "../src/audio/tone_ctrl.c" 3 4
            __builtin_sinf
# 206 "../src/audio/tone_ctrl.c"
                (w0);
    alpha = sin_w0 / (2.0f * Q);
                         ...
                         int samples )
{
    ...
    for (int ch = 0; ch < in_buf_ch; ch++)
    {
        ...
        for (int sample_idx = 0; sample_idx < samples; sample_idx++)
        {
            ...
            float y = 
# 284 "../src/audio/tone_ctrl.c" 3 4
                     fmaf
# 284 "../src/audio/tone_ctrl.c"
                        (b0, x, z1);
            z1 = 
# 285 "../src/audio/tone_ctrl.c" 3 4
                fmaf
# 285 "../src/audio/tone_ctrl.c"
                   (b1, x, 
# 285 "../src/audio/tone_ctrl.c" 3 4
                           fmaf
# 285 "../src/audio/tone_ctrl.c"
                              (-a1, y, z2));
            z2 = 
# 286 "../src/audio/tone_ctrl.c" 3 4
                fmaf
# 286 "../src/audio/tone_ctrl.c"
                   (b2, x, -a2*y);
# 298 "../src/audio/tone_ctrl.c"
            out[idx] = x + (y - x);
        }
