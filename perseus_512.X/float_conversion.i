# 1 "../src/audio/float_conversion.c"
...
# 3 "../src/audio/float_conversion.c" 2
...
float y1f(float);
float ynf(int, float);
# 9 "../src/audio/float_conversion.c" 2
...
# 11 "../src/audio/float_conversion.c" 2
...
extern void convert_float_to_pwm_20bit( const float* float_in,
                                              ...
                                              uint32_t pwm_period );
# 14 "../src/audio/float_conversion.c" 2
# 41 "../src/audio/float_conversion.c"
float Pre_Gain_CODEC;
float Post_Gain_PWM;
float Post_Gain_CODEC;
float Pre_Gain_WAV;
# 54 "../src/audio/float_conversion.c"
void convert_tdm_init( void )
{
    Pre_Gain_CODEC = powf((10.0f), (( 0.0f)) / 20.0f);
    Post_Gain_CODEC = powf((10.0f), (( 0.0f)) / 20.0f);
    Post_Gain_PWM = powf((10.0f), (( 12.0f)) / 20.0f);
    Pre_Gain_WAV = powf((10.0f), (( -42.0f)) / 20.0f);
...
# 91 "../src/audio/float_conversion.c"
                                 ...
                                 int frameSize )
{
    ...
    const int Cout = channels_out;
    const int read_ch = (Cin < Cout) ? Cin : Cout;
    ...
    for (int n = 0; n < frameSize; n++)
    {
        ...
        {
            float x = (float)in_ptr[ch] * (1.0f / 2147483648.0f) * Pre_Gain_CODEC;
            if (x < lo) x = lo;
            if (x > hi) x = hi;
            out_ptr[ch] = x;
        }
...
# 162 "../src/audio/float_conversion.c"
                                 ...
                                 int frameSize )
{
    ...
    const int Cin = channels_in;
    const int Cout = channels_out;
    const int write_ch = (Cin < Cout) ? Cin : Cout;
    ...
    for (int n = 0; n < frameSize; n++)
    {
        ...
        for (int ch = 0; ch < write_ch; ch++)
        {
            ...
            int32_t q31 = (int32_t)(x * (2147483648.0f));
...
# 224 "../src/audio/float_conversion.c"
                                       ...
                                       uint32_t per_count )
{
    ...
    float duty_min = 256.0f * 1.05f;
    float duty_max = per_count * 0.95f;
    float duty_range = duty_max - duty_min;

    for (size_t sample_idx = 0; sample_idx < num_samples; sample_idx++)
    {

        float data = float_in[sample_idx * channels_in +slot] * Post_Gain_PWM;
        ...
        {
            output[sample_idx*up_sample_factor + k] = (uint32_t)(duty_f + 0.5f);
        }
    }
}
